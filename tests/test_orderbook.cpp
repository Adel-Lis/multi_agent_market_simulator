//
// Adel Lis created test_orderbook on 21/09/2026.
//
...
#define REQUIRE(expr)                                                       \
    do {                                                                    \
        ...
        if (!(expr)) {                                                      \
            ++g_failures;                                                   \
            std::cout << "      " << __FILE__ << ':' << __LINE__            \
                      << "  REQUIRE(" #expr ") failed, test aborted\n";     \
            return;                                                         \
        }                                                                   \
    } while (0)
...
Order make_order(std::uint64_t id, std::uint32_t trader, Side side,
    ...
    return Order{.id = id, .trader_id = trader, .side = side,
...
void test_buy_sweeps_two_levels() {
    ...
    for (const Trade& t : trades) CHECK(t.buyer_id == 107);
...
void test_sell_fills_then_rests_remainder() {
    ...
    for (const Trade& t : trades) CHECK(t.seller_id == 107);
...
int main() {
    struct Test {
        ...
        void (*fn)();          // pointer to a function taking nothing, returning nothing
    };
    ...
    std::cout << '\n' << g_checks << " checks, " << g_failures << " failed, "
              << failed_tests << " of " << std::size(tests) << " tests failed\n";
